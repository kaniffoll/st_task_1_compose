...
fun Navigation(){
    ...
    NavHost(navController = navController, startDestination = Screen.PostList.name) {
        ...
        composable(
            route = Screen.PostScreen.name + "/{index}/{isLikeClicked}",
            arguments = listOf(
                navArgument("index"){
                    type = NavType.IntType
                    defaultValue = 0
                },
                navArgument("isLikeClicked"){
                    type = NavType.BoolType
                    defaultValue = false
                }
            )
        ){
            PostScreen(navController = navController, index = it.arguments?.getInt("index") ?: 0, isLikeClicked = it.arguments?.getBoolean("isLikeClicked") ?: false)
        }
